...
main( int argc, char *argv[ ] )
{
	...
	FILE *fp;
#ifdef WIN32
	errno_t err = fopen_s( &fp, "proj06cl", "r" );
	if( err != 0 )
#else
...
#endif
	{
	fprintf(stderr, "can't openCL file");
}
	...
	cl_mem dC = clCreateBuffer(Context, CL_MEM_READ_WRITE, cSize, NULL, &status );
		if(status != CL_SUCCESS) {
			fprintf(stderr, "clCreateBuffer failed (3)\n");
		}
...
// ?????
	...
	if(status !=CL_SUCCESS){
		fprintf(stderr, "CL SET KERNEL ARG dA FAILED");
	}

	status = clSetKernelArg(Kernel, 1, sizeof(cl_mem), &dB);
	if(status !=CL_SUCCESS){
		fprintf(stderr, "CL SET KERNEL ARG dB FAILED");
	}
	status = clSetKernelArg(Kernel, 2, sizeof(cl_mem), &dMW);
	if(status !=CL_SUCCESS){
		fprintf(stderr, "CL SET KERNEL ARG dMW FAILED");
	}
	status = clSetKernelArg(Kernel, 3, sizeof(cl_mem), &dC);
	if(status !=CL_SUCCESS){
		fprintf(stderr, "CL SET KERNEL ARG dC FAILED");
	}
	if(status !=CL_SUCCESS){
		fprintf(stderr, "CL SET KERNEL ARG FAILED");
	}
...
#else
	//fprintf( stderr, "%6d x %6d %4d x %4d %10.2lf\n",
